	...
	try {
		...
		if (testres?.data?.locations?.nodes?.length > 0) {
			...
			if (destinationsArr.length > 0) {
				...
				let distanceApiUrl = `https://click-n-collect-duplicate-ff6e8847a438.herokuapp.com/api/distance?customerlocation=${customerLocation}&shop=${location.hostname}`;
...
async function get_inv_locations( product) {     
	try {       
		...
		let response = await fetch(`https://click-n-collect-duplicate-ff6e8847a438.herokuapp.com/api/cart?product_id=${product.product_id}&shop=${location.hostname}`);
		if (response.ok) {
